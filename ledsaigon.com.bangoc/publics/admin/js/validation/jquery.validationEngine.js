...
(function($) {
	...
	if($.validationEngineLanguage){				// IS THERE A LANGUAGE LOCALISATION ?
 	...
 	settings = jQuery.extend({
		...
		ajaxSubmit: false,
		scroll:true,
		promptPosition: "centerLeft",	// OPENNING BOX POSITION, IMPLEMENTED: topLeft, topRight, bottomLeft, centerRight, bottomRight
		success : false,
...
$.validationEngine = {
	...
	validateCall : function(caller,rules) {	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
		...
		function content(caller,rules,position){    	  // VALIDATE LENGTH
			callerType = $(caller).attr("type");
			if (callerType == "text" || callerType == "password" || callerType == "textarea"){
								
				if(!$(caller).val()){
					$.validationEngine.isError = true;
					promptText += $.validationEngine.settings.allrules[rules[i]].alertText+ rules[position+1] + "<br />";
				}	
			}	
		}
