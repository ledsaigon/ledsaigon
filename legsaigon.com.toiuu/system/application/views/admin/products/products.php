...
<select name="search_by_category"  id="search_by_category" >
    <option value="<?php echo base_url()?>AdminCP/products/main">- - - Tất cả - - -</option>
   <?php if($listCategory){
        ...
        foreach($listCategory as $category)
		{
			...
			if($subCategory){
				foreach($subCategory as $subItems){
					...
					$subSubCategory = $proCate->getSubCateFromPid($subItems->id);
			if($subSubCategory){
				foreach($subSubCategory as $subSubItems){
        			echo "<option value=\"".$url.'/'.$subSubItems->id."\"".($subSubItems->id == $cat_id?" selected":"").">&nbsp;&nbsp;&nbsp;&nbsp;|--".$subSubItems->vn_name."</option>";				
				}
			}				
				}
			}								
                        ...
                        <td class="button_action">
                            <a href="<?php echo base_url() ?>AdminCP/products/detail/<?php echo $row->id ?>" class="edit_item" rel="<?php echo $row->id ?>" title="<?php echo lang('edit')?>"></a>
                             ...
                             <a href="javascript:formSubmit('frmAction','disable',<?php echo $row->id;?>);" class="disable_item" title="<?php echo lang('hide')?>"><?php echo lang('hide')?></a>
                              ...
                              <?php if($row->home == 0) {?>
                              <a href="javascript:formSubmit('frmAction','enable_home',<?php echo $row->id;?>);" class="enable_home"  title="Hiển thị sản phẩm bán chạy">Sản phẩm bán chạy</a>
                              <?php }else {?>
                              <a href="javascript:formSubmit('frmAction','delete_home',<?php echo $row->id;?>);" class="delete_home" title="Delete sản phẩm bán chạy">Delete sản phẩm bán chạy</a>
                              <?php }?>
                        </td>
